...
def generate_final_subset(
    ...
    for speaker_df in grouped_by_speaker:
        ...
        if noise_meta.shape[0] < clips_per_speaker:
            replace = True
        noise_clips = noise_meta.sample(n=clips_per_speaker, replace=replace).to_dict(
            "records"
        )
        for i, row in enumerate(clips.iterrows()):
            ...
            sf.write(
                os.path.join(output_dataset, split_dir_name_clean, output_filename),
                data=clean_clip,
                samplerate=16000,
            )

    output_meta.to_csv(
        os.path.join(output_dataset, f"metadata_{split_name}.csv"), index=False
    )


def generate_artifical_noise_final_subset(
    clean_inter: str,
    output_dataset: str,
    split_name: str,
    speakers_num: int,
    ...
    cat_fun_mapping = {"white": white_noise, "pink": pink_noise, "blue": blue_noise}
...
def generate_long_only_artifical_noise_final_subset(
    clean_inter: str,
    output_dataset: str,
    split_name: str,
    speakers_num: int,
...
def generate_final_subset_match_all(
    noise_inter: str,
    clean_inter: str,
    output_dataset: str,
    split_name: str,
        ...
        for i, row in enumerate(clips.iterrows()):
            ...
            if len(noise_clip) > len(clean_clip):
                ...
                noise_clip = np.concatenate(
                    [
                        noise_clip,
                        np.zeros(
                            len(clean_clip) - len(noise_clip), dtype=noise_clip.dtype
                        ),
                    ]
                )
            ...
            for j, snr in enumerate(SNRs):
                ...
                output_path = os.path.join(
                    output_dataset, split_dir_name, output_filename
                )
