...
class AddSingletonDepth(tf.keras.layers.Layer):
    def call(self, x, mask=None):
        x = tf.keras.backend.expand_dims(x, -1)  # add a dimension of the right

        if tf.keras.backend.ndim(x) == 4:
            return tf.keras.backend.permute_dimensions(x, (0, 3, 1, 2))
        else:
            return x
...
class Slice(tf.keras.layers.Layer):
    ...
    def call(self, x, mask=None):
        ...
        if (
            len(self.selector) == 2
            and not type(self.selector[1]) is slice
            and not type(self.selector[1]) is int
        ):
