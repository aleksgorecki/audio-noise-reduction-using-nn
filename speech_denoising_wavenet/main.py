...
def training(config, cla):
    ...
    if bool(config["model"].get("no_conditioning")):
        train_set_generator = dataset.get_random_batch_generator_without_conditioning(
            "train"
        )
        test_set_generator = dataset.get_random_batch_generator_without_conditioning(
            "test"
        )
    else:
        train_set_generator = dataset.get_random_batch_generator_with_conditioning(
            "train"
        )
        test_set_generator = dataset.get_random_batch_generator_with_conditioning(
            "test"
        )
...
def inference(config, cla):
    ...
    if cla.noisy_input_path.endswith(".wav"):
        ...
        filenames = [
            filename
            for filename in os.listdir(cla.noisy_input_path)
            if filename.endswith(".wav")
        ]

    clean_input = None
    for filename in filenames:
        noisy_input = util.load_wav(
            cla.noisy_input_path + filename, config["dataset"]["sample_rate"]
        )
        if cla.clean_input_path is not None:
            if not cla.clean_input_path.endswith("/"):
                cla.clean_input_path += "/"
            clean_input = util.load_wav(
                cla.clean_input_path + filename, config["dataset"]["sample_rate"]
            )
        ...
        if bool(cla.one_shot):
            if (
                len(input["noisy"]) % 2 == 0
            ):  # If input length is even, remove one sample
