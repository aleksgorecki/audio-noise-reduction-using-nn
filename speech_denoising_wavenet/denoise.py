...
def denoise_sample(
    ...
    for batch_i in tqdm.tqdm(range(0, num_batches)):
        ...
        denoised_output_fragments = model.denoise_batch(
            {"data_input": input_batch, "condition_input": condition_batch}
        )
    ...
    if input["clean"] is not None:
        ...
        output_denoised_filename = (
            output_filename_prefix + "denoised_%ddB.wav" % new_snr_db
        )
        output_noisy_filename = (
            output_filename_prefix + "noisy_%ddB.wav" % initial_snr_db
        )
    else:
