...
class ControlWidget(QWidget):
    def __init__(self, visual_widget: VisualWidget):
        ...
        self.model_session_button = QPushButton(
            "Select", parent=self.model_session_group
        )
        ...
        self.best_weights_radio = QRadioButton(
            "Use best weights", parent=self.weights_group
        )
        self.latest_weights_radio = QRadioButton(
            "Use latest weights", parent=self.weights_group
        )
        self.custom_weights_radio = QRadioButton(
            "Use selected weights", parent=self.weights_group
        )
        ...
        self.metrics_checkbox = QCheckBox(
            "Calculate metrics", parent=self.general_groupbox
        )
        self.save_result_checkbox = QCheckBox(
            "Save result", parent=self.general_groupbox
        )
        self.show_extra_checkbox = QCheckBox(
            "Show extra options", parent=self.general_groupbox
        )
    ...
    def noisy_input_onclick(self):
        ...
        noisy_input = QFileDialog.getOpenFileName(
            parent=self,
            caption=self.tr("Select a file to denoise"),
            options=options,
            filter=self.tr("Wav File (*.wav)"),
            directory=directory,
        )[0]
        if noisy_input != "":
            self.noisy_input_path = noisy_input
            self.last_noisy_dir = os.path.dirname(noisy_input)
            self.noisy_input_data = librosa.core.load(
                self.noisy_input_path, sr=16000, mono=True
            )
            ControlWidget.set_label_text_enable(
                self.noisy_input_label, str(pathlib.Path(self.noisy_input_path).name)
            )
    ...
    def session_onclick(self):
        ...
        session_dir = QFileDialog.getExistingDirectory(
            self, "Select session directory", directory=directory
        )
        if session_dir != "":
            ...
            ControlWidget.set_label_text_enable(
                self.model_session_label, str(pathlib.Path(self.session_path).name)
            )
    ...
    def select_weights_onclick(self):
        options = QFileDialog.Options()
        weights = QFileDialog.getOpenFileName(
            parent=self,
            caption=self.tr("Select weights file"),
            directory=os.path.join(self.session_path, "checkpoints"),
            options=options,
            filter=self.tr("HDF5 File (*.hdf5)"),
        )[0]
        if weights != "":
            self.custom_weights_path = weights
            ControlWidget.set_label_text_enable(
                self.weights_label, str(pathlib.Path(self.custom_weights_path).name)
            )
    ...
    def load_model_onclick(self):
        try:
            with open(os.path.join(self.session_path, "config.json"), "r") as cf:
                ...
                checkpoint = None
                if self.best_weights_radio.isChecked():
                    checkpoint = ControlWidget.get_best_checkpoint(
                        os.path.join(self.session_path, "checkpoints")
                    )
                elif self.latest_weights_radio.isChecked():
                    checkpoint = ControlWidget.get_latest_checkpoint(
                        os.path.join(self.session_path, "checkpoints")
                    )
                elif (
                    self.custom_weights_radio.isChecked()
                    and self.custom_weights_path != ""
                ):
    ...
    def run_prediction_onclick(self):
        ...
        predicted, metrics, clean_aligned = predict_example(
            self.noisy_input_data[0],
            clean_data[0],
            self.model,
            self.metrics_checkbox.isChecked(),
        )
